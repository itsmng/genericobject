...
class PluginGenericobjectObject extends CommonDBTM
{
    ...
    function showForm($id, $options = [], $previsualisation = false)
    {
        ...
        if ($previsualisation) {
            echo "<h2>" . __("Object preview", "genericobject") . ": ";
        ...
        foreach (PluginGenericobjectSingletonObjectField::getInstance($this->objecttype->fields['itemtype'])
            ...
            if ($field == "is_helpdesk_visible" && $id <= 0) {
                $ffield = $this->displayField($canedit, $field, 1, $template, $description);
                if ($ffield) {
                    $form_items[$ffield[0]] = $ffield[1];
                }
            } else {
                $ffield = $this->displayField($canedit, $field, $this->fields[$field], $template, $description);
                if ($ffield) {
                    $form_items[$ffield[0]] = $ffield[1];
                }
            }
        ...
        // We disable the action buttons when in preview mode.
        if ($previsualisation) {
            $form["buttons"] = [null];
        }
    ...
    {
        global $GO_BLACKLIST_FIELDS, $CFG_GLPI;
        ...
        return [$searchoption["name"], $form_field];
    }
        ...
        if ($this->fields['id'] > 0 && $this->fields['entities_id'] != $new_entity) {
            ...
            foreach (PluginGenericobjectSingletonObjectField::getInstance(get_called_class()) as $field => $data) {
                ...
                if (
                    ...
                    //do not go further
                    if (
                        !$dropdown->isEntityAssign()
                        || in_array($new_entity, getAncestorsOf(
                            'glpi_entities',
